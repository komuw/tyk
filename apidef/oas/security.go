...
func (s *OAS) fillToken(api apidef.APIDefinition) {
	ss := s.GetTykExtension().Server.Authentication.SecuritySchemes
	if ss == nil {
	...
	s.GetTykExtension().Server.Authentication.SecuritySchemes[authConfig.Name] = token
}
...
func (s *OAS) extractTokenTo(api *apidef.APIDefinition, name string) {
	...
	s.extractSecuritySchemeTo(&authConfig)
...
func (s *OAS) extractSecuritySchemeTo(ac *apidef.AuthConfig) {
	if len(s.Security) == 0 {
		return
	}
